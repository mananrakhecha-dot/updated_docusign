...
export interface Envelope {
  ...
  subject: string;
  message: string;
  status: 'DRAFT' | 'SENT' | 'DELIVERED' | 'COMPLETED' | 'DECLINED' | 'VOIDED' | 'TAMPERED';
  created_at: string;
  completed_at?: string;
...
export const envelopeApi = {
  ...
  create: (formData: FormData) =>
    api.post<{ envelopeId: string; documentId: string; pageCount: number }>('/envelopes', formData, {
  ...
  void: (id: string, reason: string) => api.post(`/envelopes/${id}/void`, { reason }),
...
export const signingApi = {
  ...
  complete: (token: string, signatureData: Record<string, string>, otpCode?: string) =>
    api.post(`/sign/${token}/complete`, { signature_data: signatureData, otp_code: otpCode }),
...
export const adminApi = {
  uploadId: (file: File) => {
    ...
    return api.post('/admin/id-upload', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
  },
  listReviews: () => api.get('/admin/id-reviews'),
  imageUrl: (uploadId: string) => `/api/admin/id-reviews/${uploadId}/image`,
  approve: (uploadId: string) => api.post(`/admin/id-reviews/${uploadId}/approve`),
  reject: (uploadId: string, reason: string) => api.post(`/admin/id-reviews/${uploadId}/reject`, { reason }),
